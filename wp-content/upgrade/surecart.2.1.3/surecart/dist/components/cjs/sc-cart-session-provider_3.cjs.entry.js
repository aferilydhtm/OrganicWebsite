...
const ScCartSessionProvider = class {
  ...
  async update(data = {}, query = {}) {
    var _a;
    try {
      this.session = (await index$1.updateCheckout({
  ...
  render() {
    return (index.h("sc-line-items-provider", { order: this.order, onScUpdateLineItems: e => this.loadUpdate({ line_items: e.detail }) }, index.h("slot", null)));
  }
...
const ScFormErrorProvider = class {
  ...
  render() {
    return !!this.errorMessage() ? (index.h("sc-alert", { exportparts: "base, icon, text, title, message, close", type: "danger", scrollOnOpen: true, open: !!this.errorMessage() }, index.h("span", { slot: "title" }, this.errorMessage()))) : null;
  }
  static get watchers() { return {
