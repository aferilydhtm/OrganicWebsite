...
const ScProductLineItem = class {
  ...
  render() {
    return (index.h("div", { class: "base", part: "base" }, index.h("div", { part: "product-line-item", class: { 'item': true, 'item--has-image': !!this.imageUrl, 'item--is-rtl': pageAlign.isRtl() } }, !!this.imageUrl && index.h("img", { part: "image", src: media.sizeImage(this.imageUrl, 130), class: "item__image" }), index.h("div", { class: "item__text", part: "text" }, index.h("div", { class: "item__title", part: "title" }, index.h("slot", { name: "title" }, this.name)), this.editable && (index.h("sc-quantity-select", { max: this.max || Infinity, exportparts: "base:quantity__base, minus, minus-icon, plus, plus-icon, input", clickEl: this.el, quantity: this.quantity, onScChange: e => e.detail && this.scUpdateQuantity.emit(e.detail) })), !this.editable && this.quantity > 1 && (index.h("span", { class: "item__description", part: "static-quantity" }, wp.i18n.__('Qty:', 'surecart'), " ", this.quantity))), index.h("div", { class: "item__suffix", part: "suffix" }, this.removable ? index.h("sc-icon", { exportparts: "base:remove-icon__base", class: "item__remove", name: "x", onClick: () => this.scRemove.emit() }) : index.h("div", null), this.renderPriceAndInterval())), (this.fees || []).map(fee => {
      if (this.trialDurationDays && !this.setupFeeTrialEnabled && fee.fee_type === 'setup')
        return null;
      return (index.h("sc-line-item", null, index.h("sc-format-number", { slot: "price-description", type: "currency", value: fee === null || fee === void 0 ? void 0 : fee.amount, currency: this.currency || 'usd' }), index.h("span", { slot: "price-description", class: "fee__description" }, fee === null || fee === void 0 ? void 0 : fee.description)));
    })));
  }
...
const ScQuantitySelect = class {
  ...
  render() {
    return (index.h("div", { part: "base", class: {
      ...
      } }, index.h("span", { part: "minus", role: "button", "aria-label": wp.i18n.__('decrease number', 'surecart'), class: { 'button__decrease': true, 'button--disabled': this.quantity <= this.min && this.min > 1 }, onClick: () => this.quantity > this.min && this.decrease() }, index.h("sc-icon", { name: "minus", exportparts: "base:minus__icon" })), index.h("input", { part: "input", class: "input__control", ref: el => (this.input = el), step: "1", type: "number", max: this.max, min: this.min, value: this.quantity, autocomplete: "off", tabindex: "0", role: "spinbutton", "aria-valuemax": this.max, "aria-valuemin": this.min, "aria-valuenow": this.quantity, "aria-disabled": this.disabled, onChange: () => this.handleChange(), onInput: () => this.handleInput(), onFocus: () => this.handleFocus(), onBlur: () => this.handleBlur() }), index.h("span", { part: "plus", role: "button", "aria-label": wp.i18n.__('increase number', 'surecart'), class: { 'button__increase': true, 'button--disabled': this.quantity >= this.max }, onClick: () => this.quantity < this.max && this.increase() }, index.h("sc-icon", { name: "plus", exportparts: "base:plus__icon" }))));
  }
  get el() { return index.getElement(this); }
};
