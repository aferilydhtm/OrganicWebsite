...
const ScCartButton = class {
  ...
  order() {
    return watchers.getCheckout(this.formId, this.mode);
  }
  ...
  componentDidLoad() {
    ...
    watchers.store.onChange(this.mode, () => this.handleParentLinkDisplay());
  }
  handleParentLinkDisplay() {
    this.link.style.display = !this.cartMenuAlwaysShown && !this.getItemsCount() ? 'none' : null;
  }
  render() {
    return (index.h("div", { class: "cart__button", part: "base" }, index.h("div", { class: "cart__content" }, (this.showEmptyCount || !!this.getItemsCount()) && (index.h("span", { class: "cart__count", part: "count" }, this.getItemsCount())), index.h("div", { class: "cart__icon" }, index.h("slot", null)))));
  }
