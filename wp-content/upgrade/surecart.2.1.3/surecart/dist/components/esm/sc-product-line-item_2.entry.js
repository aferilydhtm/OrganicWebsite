...
const ScQuantitySelect = class {
  ...
  render() {
    return (h("div", { part: "base", class: {
      ...
      } }, h("span", { part: "minus", role: "button", "aria-label": wp.i18n.__('decrease number', 'surecart'), class: { 'button__decrease': true, 'button--disabled': this.quantity <= this.min && this.min > 1 }, onClick: () => this.quantity > this.min && this.decrease() }, h("sc-icon", { name: "minus", exportparts: "base:minus__icon" })), h("input", { part: "input", class: "input__control", ref: el => (this.input = el), step: "1", type: "number", max: this.max, min: this.min, value: this.quantity, autocomplete: "off", tabindex: "0", role: "spinbutton", "aria-valuemax": this.max, "aria-valuemin": this.min, "aria-valuenow": this.quantity, "aria-disabled": this.disabled, onChange: () => this.handleChange(), onInput: () => this.handleInput(), onFocus: () => this.handleFocus(), onBlur: () => this.handleBlur() }), h("span", { part: "plus", role: "button", "aria-label": wp.i18n.__('increase number', 'surecart'), class: { 'button__increase': true, 'button--disabled': this.quantity >= this.max }, onClick: () => this.quantity < this.max && this.increase() }, h("sc-icon", { name: "plus", exportparts: "base:plus__icon" }))));
  }
  get el() { return getElement(this); }
};
